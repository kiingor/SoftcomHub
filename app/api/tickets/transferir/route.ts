...
import { canTransferTicket } from '@/lib/transfer-authorization'
...
export async function POST(request: Request) {
  try {
    ...
    const { data: actor, error: actorError } = await supabase
      .from('colaboradores')
      .select('id, nome, ativo, is_master, setor_id, permissoes(can_see_all_tickets)')
      .eq('email', user.email)
      .maybeSingle()
    ...
    if (ticket.colaborador_id !== actor.id && actor.is_master !== true && actorCanSeeAllTickets) {
      const { data: actorSetorLinks } = await supabase
    ...
    if (body.colaborador_id) {
      ...
      const heartbeatFresco = Boolean(
        colaborador.last_heartbeat
        && Date.now() - new Date(colaborador.last_heartbeat).getTime() < HEARTBEAT_STALE_MS,
      )
      const colaboradorDisponivel = colaborador.is_online === true && heartbeatFresco && !colaborador.pausa_atual_id
